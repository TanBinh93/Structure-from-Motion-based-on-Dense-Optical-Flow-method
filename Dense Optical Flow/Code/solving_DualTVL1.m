...
% for i = 1:dx
%     for j=1:dy
%             C0(:, :, i, j) = Ct(:, :, i, j) - u0.*Cx(:, :, i, j) - v0.*Cy(:, :, i, j);
%             s_Cx_Cx = s_Cx_Cx + Cx(:, :, i, j).*Cx(:, :, i, j)/dx/dy;
%             s_Cx_Cy = s_Cx_Cy + Cx(:, :, i, j).*Cy(:, :, i, j)/dx/dy;
%             s_Cy_Cy = s_Cy_Cy + Cy(:, :, i, j).*Cy(:, :, i, j)/dx/dy;                
% 
%             s_Cx_C0 = s_Cx_C0 + Cx(:, :, i, j).*C0(:, :, i, j)/dx/dy;
%             s_Cy_C0 = s_Cy_C0 + Cy(:, :, i, j).*C0(:, :, i, j)/dx/dy;
%     end
% end
...
for k = 1:maxits
    ...
    apply_linear_operator(K2_u_, u_);
    apply_linear_operator(K2_v_, v_);
    ...
    apply_adjoint_linear_operator(K2_s_qu, qu);
    apply_adjoint_linear_operator(K2_s_qv, qv);
